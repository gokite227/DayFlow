...
- 실행 스크립트는 apps/web(dev/build/lint/typecheck), packages/domain·packages/schemas(build/typecheck/test), packages/api-client(openapi:export/generate/typecheck/test)에만 있다. 나머지 패키지에는 스크립트가 없으므로, 해당 패키지에서 Turbo가 0개 작업을 성공한 것은 기능 테스트 성공이 아니다.
