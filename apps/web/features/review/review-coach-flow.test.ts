...
describe("Where the Review Coach lives", () => {
  it("is part of the writing view only, never the archive", async () => {
    const { readFileSync } = await import("node:fs");
    const archive = readFileSync(new URL("./review-archive-view.tsx", import.meta.url), "utf8");
    const writing = readFileSync(new URL("./review-view.tsx", import.meta.url), "utf8");
    expect(archive).not.toMatch(/ReviewCoach|coach/i);
    expect(writing).toContain("<ReviewCoachCard");
  });
});
