...
import { addDays, startOfWeek, toInstant } from "../calendar/calendar-time";
import {
...
const schedule = (date: string, start: number, length: number): NonNullable<DayResponse["schedule"]> => ({
  id: "schedule-1",
  dayId: "day-1",
  startAt: toInstant(date, start),
  endAt: toInstant(date, start + length),
  timezone: "Asia/Seoul",
  createdAt: "2031-03-01T00:00:00Z",
  updatedAt: "2031-03-01T00:00:00Z",
  version: 7,
...
describe("planning apply plans", () => {
  it("moves a schedule keeping its length", () => {
    ...
    expect(plan).toMatchObject({
      kind: "schedule",
      body: { startAt: toInstant("2031-03-05", 600), endAt: toInstant("2031-03-05", 690), expectedVersion: 7 },
    });
  });
