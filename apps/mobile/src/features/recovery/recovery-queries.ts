import type { ApplyCarryOverRequest, ApplyRecoveryRequest, CarryOverPreviewRequest, SaveRecoveryDayRequest } from "@dayflow/api-client";
...
/** REC-001 missed Days as of the device's today and now (key under "days": every Day change refreshes it). */
export function useRecoveryCandidates(today: string) {
  return useQuery({
    queryKey: queryKeys.days.recoveryCandidates(today),
    queryFn: () =>
      expectData(getDayFlowApiClient().GET("/api/v1/recovery/candidates", { params: { query: { today, now: new Date().toISOString() } } })),
...
export function useApplyRecovery() {
  ...
  return useMutation({
    mutationFn: (body: ApplyRecoveryRequest) => expectData(getDayFlowApiClient().POST("/api/v1/recovery/apply", { body })),
    onSettled: () => {
...
/** REC-004 preview: read-only, keyed by everything the user chose. */
...
export function useApplyCarryOver() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (body: ApplyCarryOverRequest) => expectData(getDayFlowApiClient().POST("/api/v1/recovery/carry-over/apply", { body })),
    onSettled: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.days.all });
...
export function useRecoveryDays(from: string, to: string) {
  return useQuery({
    ...
    queryFn: () => expectData(getDayFlowApiClient().GET("/api/v1/recovery-days", { params: { query: { from, to } } })),
  });
}
...
export function useDeleteRecoveryDay() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (date: string) => expectNoContent(getDayFlowApiClient().DELETE("/api/v1/recovery-days/{date}", { params: { path: { date } } })),
