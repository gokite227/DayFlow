import { isCalendarGoalResponse, isPeriodGoalResponse, type GoalResponse, type PeriodGoalResponse } from "@dayflow/api-client";
...
import { toCreatePeriodGoalRequest, toUpdatePeriodGoalRequest, type PeriodGoalFormValues } from "./period-goal-helpers";

export function useGoals() {
  return useQuery({
    queryKey: queryKeys.goals.list(),
    queryFn: async () => {
      const goals = await expectData(getDayFlowApiClient().GET("/api/v1/goals", { params: { query: { kind: "CALENDAR" } } }));
...
export function useGoal(goalId: string | undefined) {
  return useQuery({
    ...
    queryFn: () => expectData(getDayFlowApiClient().GET("/api/v1/goals/{goalId}", { params: { path: { goalId: goalId ?? "" } } })),
...
// PERIOD Goals are not part of the CALENDAR list, so only the period list and the detail are refreshed.
...
export function useUpdatePeriodGoal() {
  ...
  return useMutation({
    mutationFn: async ({ goal, values }: { goal: PeriodGoalResponse; values: PeriodGoalFormValues }) =>
      asPeriodGoal(
        await expectData(
          getDayFlowApiClient().PATCH("/api/v1/goals/{goalId}", { params: { path: { goalId: goal.id } }, body: toUpdatePeriodGoalRequest(values, goal.version) }),
        ),
      ),
    onSuccess: (goal) =>
      Promise.all([
    ...
    onError: (_error, { goal }) => void queryClient.invalidateQueries({ queryKey: queryKeys.goals.detail(goal.id) }),
  });
}
...
export function useDeletePeriodGoal() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (goalId: string) => expectNoContent(getDayFlowApiClient().DELETE("/api/v1/goals/{goalId}", { params: { path: { goalId } } })),
