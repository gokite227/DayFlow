...
import { durationMinutes, scheduleRequest } from "../calendar/calendar-grid";
...
/** "19:30" → minutes of the day; null for anything else. */
...
export function planSuggestionApply(
  ...
  if (suggestion.type === "SET_SCHEDULE" && suggestion.targetDate && suggestion.startTime) {
    const start = parseClock(suggestion.startTime);
  ...
  return {
    ...
    async applySuggestion(suggestion, index) {
      ...
      try {
        ...
        if (!(await deps.confirm(plan.confirmMessage))) {
          set({ applying: null });
          return;
        }
    ...
    async createProposal(proposal, index) {
      ...
      set({ applying: key, applyError: null });
      if (!(await deps.confirm(`"${proposal.title}" Day를 ${when}새로 만들까요?`))) {
        set({ applying: null });
        return;
      }
      try {
