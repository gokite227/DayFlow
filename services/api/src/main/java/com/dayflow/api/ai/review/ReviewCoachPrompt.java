...
final class ReviewCoachPrompt {
    ...
    static final String INSTRUCTIONS = """
            ...
            - "때문에", "~로 인한", "~로 인해", "탓"처럼 원인을 단정하는 표현을 쓰지 않는다. 함께 나타난 사실만 나란히 말한다.
            ...
            - 이전 TRY가 지켜졌다·효과가 있었다·반영됐다고 말하려면 PREVIOUS_TIME_*, PREVIOUS_COMPLETION, PREVIOUS_OVERBOOKED 같은
              지난 기간 → 이번 기간 비교 evidence를 함께 단다. 비교 evidence가 없으면 효과를 판단하지 않는다.
            - 성공·실패를 단정할 수 없으면 "관련된 변화가 보여요", "직접 관련됐다고 단정하기는 어려워요"처럼 표현한다.

            [출력]
            - 반드시 지정된 JSON 스키마로만 답한다. 모든 문장은 한국어 해요체("~했어요", "~해 보세요")로 짧고 구체적으로 쓴다.
